...
from utils.Bert3GramDictUtils import load_data
...
def eval_dataset(model, criterion, data, device, typ, visual_logger, stamp):
    ...
    for insts, golds, dict_datas in data:
        golds = golds.to(device)
        dict_datas = dict_datas.to(device)

        pred = model(insts, golds, dict_datas)
...
def main():
    ...
    model = Bert3GramDict(config.device, config.cache_3gram_path)
    if config.use_cuda and torch.cuda.is_available():
    ...
    for epoch_i in range(config.epoch):
        for batch_i, [insts, golds, dict_datas] in enumerate(train_data):
            golds = golds.to(config.device)
            dict_datas = dict_datas.to(config.device)
            model.train()
            ...
            if steps % config.valInterval == 0:
                ...
                if F_dev > best_perf[2]:
                    ...
                    torch.save(model.pack_state_dict(), os.path.join(config.save_path, 'ctb60_dict_final.pt'))
