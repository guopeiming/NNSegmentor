...
class Bert3Gram(nn.Module):
    def __init__(self, device, cache_3gram_path):
        super(Bert3Gram, self).__init__()
        self.tokenizer = BertTokenizer.from_pretrained('bert-base-chinese')
        self.bert_model = BertModel.from_pretrained('bert-base-chinese')

        self.cnn = nn.Sequential(
            nn.Conv1d(768, 768, 7, stride=1, padding=3),
            nn.ReLU()
        )
    ...
    def forward(self, insts: List[str], golds: torch.Tensor):
        ...
        cnn_output = self.cnn(self.dropout1(bert_output.permute(0, 2, 1)))  # (batch_size, 768, seq_len)
        ...
        for inst_seq_len, inst in zip(insts_seq_len, insts):
            for i in range(inst_seq_len):
                if self.training:
                    ...
                    inst_3gram = inst[i:i+3] if inst_3gram_str in self.__3gram_cache else [mask_id, inst[i+1], mask_id]
