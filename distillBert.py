...
from utils.distillBertUtils import load_data
...
def cal_preformance(pred, golds, criterion, device, criterion2, logits, Lambda):
    ...
    logits = torch.tensor(logits).to(device).view(-1, 2)
    loss = (1-Lambda)*criterion(pred, golds) + Lambda*criterion2(pred, logits)

    return loss, golds_word, pred_word, seg_word, char, cor_char


@torch.no_grad()
def eval_model(model, criterion, dev_data, test_data, device, visual_logger, stamp, criterion2, Lambda):
    model.eval()
    print('Validation starts...')
    F_dev = eval_dataset(model, criterion, dev_data, device, 'dev', visual_logger, stamp, criterion2, Lambda)
    F_test = eval_dataset(model, criterion, test_data, device, 'test', visual_logger, stamp, criterion2, Lambda)
    print('Validation ends.')
    return F_dev, F_test


def eval_dataset(model, criterion, data, device, typ, visual_logger, stamp, criterion2, Lambda):
    total_loss, golds_words, pred_words, seg_words, chars, cor_chars = 0.0, 0, 0, 0, 0, 0
    for insts, golds, logits in data:
        golds = golds.to(device)

        pred = model(insts, golds)
        loss, golds_word, pred_word, seg_word, char, cor_char = cal_preformance(pred, golds, criterion, device, criterion2, logits, Lambda)
...
def main():
    ...
    criterion2 = torch.nn.MSELoss(reduction='sum').to(config.device)
    ...
    for epoch_i in range(config.epoch):
        for batch_i, [insts, golds, logits] in enumerate(train_data):
            golds = golds.to(config.device)
            model.train()

            pred = model(insts, golds)
            loss, golds_word, pred_word, seg_word, char, cor_char = cal_preformance(pred, golds, criterion, config.device, criterion2, logits, config.distillLambda)
            total_loss += loss.item()
            golds_words += golds_word
            pred_words += pred_word
            seg_words += seg_word
            ...
            if steps % config.valInterval == 0:
                F_dev, F_test = eval_model(model, criterion, dev_data, test_data, config.device, visual_logger, steps, criterion2, config.distillLambda)
